...
# are 4 tiles wide (dark asphalt), edged by 2-tile sidewalks. District cells
# sit between roads. Flat colors; per-pixel noise variants removed during the
# pre-perspective-conversion cleanup.
...
const TILE_COLORS := [
	Color("3a3a3a"),  # street - weathered cobble/asphalt mix, ambient road fill
	Color("5e5e5e"),  # sidewalk - light grey concrete (lighter than asphalt)
...
func _build_tileset() -> void:
	# Flat solid colors. Procedural per-pixel noise was removed during Track 2
	# cleanup - it made the map read as visual chaos at any zoom.
