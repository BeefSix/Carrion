...
1. **CharacterBody2D + `move_and_slide`** — zombie/unit movement goes through Godot's physics server, which is not lockstep-deterministic and is fundamentally float-with-physics-engine-internals. The real fix is the §3.2 steering-field migration: all zombie movement becomes coarse-grid sampling + deterministic integration, no physics queries. Same applies to the unit collision response. Until that migration, position drift IS expected and the determinism CI will show it. **NOTE (2026-06-10): the `SteeringField` autoload now EXISTS but the migration has NOT happened — the field currently only biases zombie *wander direction*; locomotion still runs through `move_and_slide` (`Shambler.gd` 721/735/889). This irreducible is therefore STILL OPEN. Don't let "SteeringField shipped" elsewhere read as "physics divergence retired" — it isn't until zombie movement actually leaves the physics server.**
