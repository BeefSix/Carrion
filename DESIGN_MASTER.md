...
- **Hunter** (basic ranged) — bow; **passively binds up to 4 nearby wild zombies into a purely *defensive* escort** (decided + shipped 2026-06-10, replacing the earlier "ambient clicking cluster" concept). The thralls body-block for him but deal **zero damage** — they are the shield, never the sword; the Hunter must earn every kill on his own bow. They match his speed, hold a loose ring, stay loyal regardless of his HP, and **revert to wild the instant he dies** (per the §7.2 control rule). Binding emits **no noise** and cannot trigger hordes — that is the deliberate lesson from the over-powered clicking prototype, which fed NoiseField and gathered ~50 zombies. Counterplay: focused ranged fire reaches the Hunter *through* the escort (Military's answer), kill the Hunter to scatter it, or lure/steal wild zombies before he binds them (Saboteur). Doctrines **[PROPOSED]**: **Still-path** (forgoes the escort entirely for longer range — a true silent stalker) / **Deep-click** (a larger escort and plague-tipped arrows — a walking horde nucleus brought to Military's door).
