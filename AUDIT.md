...
> **STATUS — updated 2026-06-10 (read before acting on any finding below).** This is the 2026-06-07 baseline audit; much has been resolved since. **C1 and all of H1–H14 are FIXED** (each carries a `FIXED` tag on its line). The **D-series determinism findings (D1–D12) are complete or documented-deferred** — notably D8 (bare-RNG sweep) is done — the only remaining divergence class is `move_and_slide` physics (NETCODE.md irreducible #1, the steering-field migration's job, not a patch). That physics divergence is **intermittent** (onset varies, tick ~600–2760); per-component checksum hashing (added 2026-06-10) proved it is zombie-position-only with every logic/RNG component bit-identical — see the NETCODE.md irreducible-#1 amendment. **Decay is now visual-only** (DESIGN_MASTER §2), so any finding here that treats decay as a gameplay cost/emitter is **superseded** (see the annotated §"Decay" line). A finding is resolved only if its line carries a `FIXED`/`RESOLVED`/superseded tag — untagged Medium/Low items may still be open. Do not re-surface tagged items in new sessions.
...
- **Decay**: only the Command Post is in `decay_emitter`; the plan says *every* active Military structure emits. Military expansion via Barracks is currently decay-free, weakening feel-check #3 (decay regret). **SUPERSEDED 2026-06-10 — decay was demoted to visual-only (DESIGN_MASTER §2); decay is no longer a gameplay cost, so emitter coverage no longer matters. "Occupation has a price" is now carried by the zombie ecosystem (noise→hordes, corpses→rises), not decay.**
...
- **D6 — Formation positions computed with transcendentals.** `SelectionManager.gd:201` (`cos/sin` in `_formation_position`). These results become nav targets = sim state (Rule #5). Fix: precomputed offset table (the ring layout is fixed — 6/12/18 slots) or integer-vector approximations. `Main.gd:414` has the same pattern but is the dev-only `--repro-hg-horde` path (Low). `Unit.gd:632` is in `_draw` — sanctioned render use. FIXED 2026-06-09 — exact integer hex-ring walk + literal sqrt(3)/2 constant (commit 29a2deb); dev-only Main.gd repro path left as-is.
...
- **D12 — DecayField `_process`.** `DecayField.gd:57`. Mostly draw-cost work (viewport cull), but verify the decay *state* mutation isn't in the `_process` path before declaring it render-only. 10-minute check next time the file is touched. CHECKED + RESOLVED 2026-06-09: the grid mutation IS in _process, and two gameplay readers existed (Lootable spawn rate, Shambler wander scoring) — but DESIGN_MASTER §2 had already demoted decay to visual-only, so both reads were severed per canon (commit 29a2deb). With no gameplay readers, the _process mutation is legitimately render-only.
